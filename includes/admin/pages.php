 ...
 * 的副作用式防線。
 *
 * v25.8.84~87 版面曾改成左側垂直選單／手機版下拉選單的兩欄自訂版面，v25.8.88 起改回
 * 單欄＋nav-tab-wrapper（`twshop_render_admin_section_tabs()`），比照 WooCommerce 設定頁
 * 自己「最外層 nav-tab-wrapper、分類內子項目 subsubsub」的兩層導覽分工，詳見該函式與
 * CLAUDE.md 對應章節。改回單欄後不再需要 v25.8.85 那個額外的功能名稱 `<h2>`——
 * nav-tab-wrapper 本身的 active 樣式就足以顯示目前在哪個功能，跟 WooCommerce 自己的
 * 設定頁一致（例如「運送方式」分頁內容不會再重複印一次「運送方式」標題）。
 */
